    ...
    public class BiomeMapByChunkGenerator : IBiomeMapGenerator
    {
            ...
            float[] offsetY, float[] densities)
        {
            for (int i = 0; i < biomes.Length; i++)
            {
                ...
                float biomeValue = Noise(wx * WorldBiomeGenSettings.biomeFieldFrequency + offsetX[i],
                    wy * WorldBiomeGenSettings.biomeFieldFrequency + offsetY[i]) * biome.globalWeight;
                densities[i] = biomeValue * heightFactor;
            }
        }
